...
define([
    ...
    $.widget('mage.productGallery', {
        ...
        setBase: function(imageData) {
            let baseImage = this.options.types.image,
                sameImages = $.grep(
        ...
        _addItem: function(event, imageData) {
            let count = this.element.find(this.options.imageSelector).length,
                element,
                imgElement;
        ...
        _getRoles: function() {
            return _.mapObject(this.options.types, function(data, key) {
                let elem = this.element.find('.image-' + key);
        ...
        _updateImagesRoles: function() {
            let $images = this._getImages().toArray(),
        ...
        _updateImageDimesions: function(imgContainer) {
            let $img = imgContainer.find(this.options.imageElementSelector)[0],
        ...
        _removeItem: function(event, imageData) {
            let $imageContainer = this.findElement(imageData);
        ...
        _setPosition: function(event, data) {
            let $element = this.findElement(data.imageData),
    ...
    $.widget('mage.productGallery', $.mage.productGallery, {
        ...
        /** @inheritdoc */
        _create: function() {
            let template = this.element.find(this.options.dialogTemplate),
        ...
        _initDialog: function() {
            let $dialog = $(this.dialogContainerTmpl());
            ...
            $dialog.on('close', function() {
                let $imageContainer = $dialog.data('imageContainer');
            ...
            $dialog.on('change', '[data-role=visibility-trigger]', $.proxy(function(e) {
                let imageData = $dialog.data('imageData');
        ...
        onDialogOpen: function(event) {
            let imageData = this.$dialog.data('imageData'),
